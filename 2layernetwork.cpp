...
public:
    ...
    void train(const std::vector<TrainingSample> &samples, int num_epochs, double learning_rate)
    {
        for (int epoch = 0; epoch < num_epochs; epoch++)
        {
            ...
            {	
                ...
                std::vector<double> output = feedforward(hidden2_output);
                ...
                for (int i = 0; i < num_classes; i++)
                {
                    biases3[i] += learning_rate * output_delta[i];
                    for (int j = 0; j < hidden_layer2_size; j++)
                    {
                        weights3[j][i] += learning_rate * output_delta[i] * hidden2_output[j];
                    }
                }
            }
    ...
    NeuralNetwork(int input_size, int hidden_layer1_size, int hidden_layer2_size, int num_classes)
    {
        ...
        for (int i = 0; i < input_size; i++)
        {
            ...
            for (int j = 0; j < hidden_layer1_size; j++)
            {
                weights1[i][j] = 2.0 * rand() / RAND_MAX - 1.0;
            }
        }
        for (int i = 0; i < hidden_layer1_size; i++)
        {
            biases1[i] = 2.0 * rand() / RAND_MAX - 1.0;
        }
        ...
        for (int i = 0; i < hidden_layer1_size; i++)
        {
            ...
            for (int j = 0; j < hidden_layer2_size; j++)
            {
                weights2[i][j] = 2.0 * rand() / RAND_MAX - 1.0;
            }
        }
        for (int i = 0; i < hidden_layer2_size; i++)
        {
            biases2[i] = 2.0 * rand() / RAND_MAX - 1.0;
        }

        weights3.resize(hidden_layer2_size);
        biases3.resize(num_classes);
        for (int i = 0; i < hidden_layer2_size; i++)
        {
            weights3[i].resize(num_classes);
            for (int j = 0; j < num_classes; j++)
            {
                weights3[i][j] = 2.0 * rand() / RAND_MAX - 1.0;
            }
        }
        for (int i = 0; i < num_classes; i++)
        {
            biases3[i] = 2.0 * rand() / RAND_MAX - 1.0;
        }
        std::vector<double> feedforward(const std::vector<double> &input);
        // Backpropagation function
...
int main()
{
    ...
    NeuralNetwork neural_network(input_size, hidden_layer1_size, hidden_layer2_size, num_classes);
    // Load the training samples
    std::vector<TrainingSample> training_samples;
    for(int i = 0; i < (rng()%10000); i++)rand();
    for(int i = 0; i <600 ;i++){
        ...
        for(int j = 0; j < 25; j++){
            grid.push_back((double)((int)rand() % 2));
